...
class Player(Character):
    ...
    def check_enemy_collision(self, enemy):
        ...
        if (abs(self.actor.x - enemy.actor.x) < 30 and 
            ...
            self.score += 50  # points
            self.progress += 10  # progress
            enemy.defeat()
            ...
            if abs(self.actor.x - enemy.actor.x) < 30 and abs(self.actor.y - enemy.actor.y) < 30:
                self.take_damage()
                if self.lives <= 0:  # If you run out of lives, game over immediately.
                    global game_state
                    game_state = GAME_OVER

    def take_damage(self):
        #Handle player taking damage.
        global game_state  # We declare that we will use the global variable
...
def draw():
    ...
    if game_state == MENU:
        ...
        screen.draw.text("Adventure Kod Island", center=(WIDTH//2, HEIGHT//2 - 150),
                        color=WHITE, fontsize=64, shadow=(2, 2))
        
        draw_menu_button("Iniciar Jogo", HEIGHT//2 - 50)
        draw_menu_button(f"Musica: {'On' if music_on else 'Off'}", HEIGHT//2 + 10)
        draw_menu_button(f"Sons: {'On' if sound_on else 'Off'}", HEIGHT//2 + 70)
        draw_menu_button("Sair", HEIGHT//2 + 130)
        ...
        screen.draw.text(f"pontos: {player.score}", (10, 10),
                       color=WHITE, fontsize=32)
        
        screen.draw.text(f"Progresso: {player.progress}%", (10, 50),
                       color=WHITE, fontsize=24)
        ...
        screen.blit('background-1', (0, 0))
        screen.draw.text("Você ganhou ", center=(WIDTH//2, HEIGHT//2 - 60),
                        color=(255, 215, 0), fontsize=64, shadow=(4, 4))  # color gold
        screen.draw.text(f"Pontos: {player.score}", center=(WIDTH//2, HEIGHT//2),
                        color=WHITE, fontsize=32, shadow=(2, 2))
        screen.draw.text("Clique no X vermelho para voltar ao menu", center=(WIDTH//2, HEIGHT//2 + 40),
                        color=WHITE, fontsize=24)
        ...
        screen.blit('background-1', (0, 0))
        screen.draw.text("FIM DE JOGO", center=(WIDTH//2, HEIGHT//2), color=WHITE, fontsize=64)
        screen.draw.text("Pressione ESPAÇO para tentar novamente", center=(WIDTH//2, HEIGHT//2 + 50), color=WHITE, fontsize=32)
